...
class DownloadQueueCard extends HTMLElement {
  ...
  _itemHtml(item, type) {
    ...
    const errMsg     = item.error_message || (item.status_messages || []).map(m => m.title || m.message || '').filter(Boolean).join('; ') || null;
    ...
    <div class="q-row">
      ...
      <div class="q-body">
        ...
        <div class="q-progress-row">
          <div class="progress-bar"><div class="progress-fill" style="width:${pct != null ? pct.toFixed(1) : 0}%;background:${statusCo}"></div></div>
  ...
  _render() {
    ...
    if (!hasSource) {
      ...
      if (total === 0) {
        ...
        bodyHtml = hasBoth
          ? this._sectionHtml(sonarrItems, 'sonarr', 'Sonarr') + this._sectionHtml(radarrItems, 'radarr', 'Radarr')
          : (sonarrItems.length
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .refresh-icon:hover{color:var(--primary-color)}
        @keyframes spin{to{transform:rotate(360deg)}}
        .section-hdr{display:flex;align-items:center;gap:6px;padding:7px 14px 5px;font-size:.7em;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--secondary-text-color);background:var(--secondary-background-color,rgba(0,0,0,.03));border-bottom:1px solid var(--divider-color,rgba(0,0,0,.08));--mdc-icon-size:13px}
        .section-count{margin-left:auto;background:var(--divider-color,rgba(0,0,0,.12));color:var(--secondary-text-color);font-size:.85em;padding:1px 6px;border-radius:8px}
        .q-row{display:flex;align-items:flex-start;padding:10px 14px;gap:11px;border-bottom:1px solid var(--divider-color,rgba(0,0,0,.06))}
        .q-row:last-child{border-bottom:none}
        .poster-col{flex-shrink:0;width:38px}
        .poster{width:38px;height:57px;border-radius:4px;object-fit:cover;display:block;background:var(--secondary-background-color)}
        .poster.no-img{width:38px;height:57px;border-radius:4px;background:var(--secondary-background-color);display:flex;align-items:center;justify-content:center;--mdc-icon-size:20px;color:var(--disabled-text-color,#9e9e9e)}
        .q-body{flex:1;min-width:0}
        .q-title{font-size:.9em;font-weight:600;color:var(--primary-text-color);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px}
        .q-sub{font-size:.75em;color:var(--secondary-text-color);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:5px;font-family:monospace}
        .q-progress-row{display:flex;align-items:center;gap:6px;margin-bottom:5px}
        .progress-bar{flex:1;height:4px;background:var(--divider-color,rgba(0,0,0,.12));border-radius:2px;overflow:hidden}
        .progress-fill{height:100%;border-radius:2px;transition:width .4s}
        .pct-label{font-size:.7em;color:var(--secondary-text-color);flex-shrink:0;width:30px;text-align:right}
        .q-meta-row{display:flex;align-items:center;gap:5px;font-size:.72em;flex-wrap:wrap;color:var(--secondary-text-color)}
        .status-pill{font-weight:700;text-transform:capitalize}
        .meta-sep{opacity:.35}
        .meta-bit{color:var(--secondary-text-color)}
        .err-msg{font-size:.72em;color:var(--error-color,#f44336);margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .info-msg{padding:22px 16px;font-size:.85em;color:var(--secondary-text-color);text-align:center}
        .info-msg.error{color:var(--error-color,#f44336)}
        .info-msg code{font-size:.9em;background:var(--secondary-background-color);padding:1px 5px;border-radius:3px}
        .spin{display:inline-block;animation:spin 1s linear infinite}
...
console.info('%c RUSSTUM-CARDS %c NAS · UPS · Media · Minecraft · Docker · Device Monitor · Sonarr Upcoming · Download Queue ', 'color:#fff;background:#1976d2;font-weight:700;padding:2px 6px;border-radius:3px 0 0 3px', 'color:#1976d2;background:rgba(25,118,210,0.1);font-weight:600;padding:2px 6px;border-radius:0 3px 3px 0');
